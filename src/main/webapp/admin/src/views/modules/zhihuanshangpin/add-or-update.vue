<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="置换状态" prop="zhihuanzhuangtai">
          <el-select :disabled="ro.zhihuanzhuangtai" v-model="ruleForm.zhihuanzhuangtai" placeholder="请选择置换状态">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="置换状态" prop="zhihuanzhuangtai">
	      <el-input v-model="ruleForm.zhihuanzhuangtai"
                placeholder="置换状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="置换姓名" prop="zhihuanxingming">
          <el-input v-model="ruleForm.zhihuanxingming" 
              placeholder="置换姓名" clearable  :readonly="ro.zhihuanxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="置换姓名" prop="zhihuanxingming">
              <el-input v-model="ruleForm.zhihuanxingming" 
                placeholder="置换姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='zhihuanzhuangtai'){
            this.ruleForm.zhihuanzhuangtai = obj[o];
	    this.ro.zhihuanzhuangtai = true;
            continue;
          }
          if(o=='shangpinfenlei'){
	    ...
	    this.ro.zhihuanzhanghao = true;
          ...
          if(o=='zhihuanxingming'){
            this.ruleForm.zhihuanxingming = obj[o];
	    this.ro.zhihuanxingming = true;
            continue;
          }
          if(o=='lianxidianhua'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.zhihuanzhanghao!=''&&json.zhihuanzhanghao){
                this.ruleForm.zhihuanzhanghao = json.zhihuanzhanghao
	    		this.ro.zhihuanzhanghao = true;
		}
		if(json.zhihuanxingming!=''&&json.zhihuanxingming){
                this.ruleForm.zhihuanxingming = json.zhihuanxingming
	    		this.ro.zhihuanxingming = true;
		}
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.chengseOptions = "全新,九成新,八成新,七成新及以下".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zhihuanshangpin/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../ssm9z7y9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhihuanshangpin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhihuanshangpinCrossAddOrUpdateFlag = false;
						 this.parent.search();
