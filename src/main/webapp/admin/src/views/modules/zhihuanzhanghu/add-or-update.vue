...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if((this.ruleForm.zhihuanzhanghao.length<3)){
        this.$message.error(`置换账号长度不能小于3`);
        return
      }
      if((this.ruleForm.zhihuanzhanghao.length>10)){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhihuanzhanghu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhihuanzhanghuCrossAddOrUpdateFlag = false;
						 this.parent.search();
